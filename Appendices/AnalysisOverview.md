...
- the United Nations uses ISIC (International Standard Industrial Classification) codes

... and they are all different and they don't necessarily map to each other in a simple way.

This lack of standardization is a general problem across all data-sets which makes data integration (and I've done more than my fair share of that) a bit of a challenge.

Given all that any analysis tends to be approximately correct rather than precisely correct, but that's OK because we are not trying to be precise, we are just trying to get a general answer to general questions.
...
Basically, it's an enhanced request/response interaction in which RAG-AI "remembers" the context in which each additional statement is made which allows for an iterative exploration of a subject.

Ideally a conversation should be complete in a single session in order to preserve the context of that conversation and separate conversations should be maintained in separate sessions even if they are running in parallel.
I use Firefox Private Containers for each session to maintain the separation but separate windows )not tabs) also works quite well.

It's also very important to confirm the accuracy of any response by confirming any references that are included in the response and that a quoted source is reliable.
This is simply because the Internet contains a lot of rubbish nowadays but RAG-AI is incapable of differentiating between that rubbish and an authoritative data source.
