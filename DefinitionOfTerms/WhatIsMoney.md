...
Electronic Money, which is *not* the same as Digital Money (sse below), is really a Payment Method (same as a Cheque or Bank Note) rather than another form of Fiat Money but nowadays is far more prevalent that any other Payment Method to the extent that it appears to be some kind of Money in its own right. 
...
Pretty much everyone will have at some time or another heard some variant of "_Time is Money_"

However, having worked for an Investment Bank the importance of time (and not to waste it) was continually emphasised because Time once spent cannot ever be regained.

As a consequence is legitimate to value something based on the amount of time spent producing it as well as estimate a future cost based on the same time spent basis. 
There are more than one Time Based Economic Systems that use "Time Credits" as the basis for an Economy and in these economic systems one "Time Credit" is exactly one hour of someone's time no matter what skill set they apply to the work.
